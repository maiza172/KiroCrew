...
def materialize_workspace_dir(validated: Path, *, display: str) -> None:
    ...
    *validated* is the path AS THE CALLER'S VALIDATION RESOLVED IT (``Path.resolve()``
    ...
    The create goes through the PINNED parent where the platform can pin: every
    parent component is opened with ``O_NOFOLLOW`` relative to the previous one and
    the directory is made relative to that descriptor, so a component swapped for a
    link between validation and this mkdir is refused rather than followed. That is
    the discipline every other write under a validated path in this product keeps
    (:mod:`kiro_crew.pinned_fs`); a by-name ``mkdir`` here would be the one write
    that followed the swap. Where nothing can be pinned (Windows: no ``dir_fd``, no
    ``O_NOFOLLOW``) the create is by name, which is that module's documented limit,
